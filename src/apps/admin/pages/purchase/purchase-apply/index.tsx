...
import {
  ...
  purchaseApplyExport,
  purchaseApplyStatus,
} from "src/apps/admin/api/purchase-apply";
//关联接口
...
function PurchaseApplyList() {
  const table = useSearchTable(getPurchaseApplyList);
  ...
  function Edit(id: PurchaseApply["id"]) {
    const window = openWindow.openCurrentAppWindow(
      `/purchase/purchase-apply/edit?id=${id}`,
      "编辑采购申请"
    );
  ...
  const column = table.column([
    {
      ...
      render: (_, record) => (
        <Typography.Link
          ...
          onClick={() => {
            const window = openWindow.openCurrentAppWindow(
              `/purchase/purchase-apply/detail?id=${record.id}`,
              "采购合同详情 - " + record.code
            );

            function listener(
              event: MessageEvent<keyof PurchaseApply["btn_power"]>
            ) {
              if (
                [
                  "is_submit",
            ...
            <Button
              ...
              onClick={function () {
                contextedModal.modal?.confirm({
                  ...
                  onOk() {
                    return deletePurchaseApply({ id: row.id }).then(() => {
  ...
  const columnState = useColumnState("purchaseApplyList", column);
          ...
          <Tabs
            ...
            onChange={(e) => {
              const status =
                e === `-1`
                  ? undefined
                  : (e as unknown as PurchaseApply["status"]);
...
export default styled(PurchaseApplyList)``;
