...
>(function (props, ref) {
  ...
  function getRenderNames(
    params: Parameters<typeof salesOrderMaterialRender>[0]
  ) {
    salesOrderMaterialRender(params).then((res) => {
  ...
  const attrColumn = useMemo(() => {
    return renderNames.attr_fields.map<ProColumns<SalesOrderMaterialSku>>(
      (item) => ({
        title: item.name,
        renderText: (_, row) => row[item.key as keyof SalesOrderMaterialSku],
      })
    );
